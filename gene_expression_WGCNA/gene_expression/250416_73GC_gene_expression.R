...
### Table S10 representative 500 genes ----
load("../input/T.to.R.genes.80_Kos") # 92
load("../input/T.to.R.genes.80_Tak") # 101

T.to.R.genes.80_Kos_rnaseq <- id_converter[T.to.R.genes.80_Kos,"rnaseq"]
T.to.R.genes.80_Tak_rnaseq <- id_converter[T.to.R.genes.80_Tak,"rnaseq"]

des_genelist_T.to.R.genes.80_kos_rnaseq <- des[setdiff(T.to.R.genes.80_Kos_rnaseq,T.to.R.genes.80_Tak_rnaseq),c(2:3,8)]
des_genelist_T.to.R.genes.80_tak_rnaseq <- des[setdiff(T.to.R.genes.80_Tak_rnaseq,T.to.R.genes.80_Kos_rnaseq),c(2:3,8)]
des_genelist_T.to.R.genes.80_kos_tak_rnaseq <- des[intersect(T.to.R.genes.80_Kos_rnaseq,T.to.R.genes.80_Tak_rnaseq),c(2:3,8)]

### Table 10 
write.table(des_genelist_T.to.R.genes.80_kos_rnaseq,"../output/Table.S10_kos.tsv",
            sep="\t", row.names = F)
write.table(des_genelist_T.to.R.genes.80_tak_rnaseq,"../output/Table.S10_tak.tsv",
            sep="\t", row.names = F)
write.table(des_genelist_T.to.R.genes.80_kos_tak_rnaseq,"../output/Table.S10_kos_tak.tsv",
            sep="\t", row.names = F)
...
### Table S11 T to R genes GO analysis ----
...
glist <- list(
    Kos = names(exemplars_new)[exemplars_new %in% as.numeric(representative_474_rnaseq[T.to.R.genes.80_Kos_rnaseq])],
    Tak = names(exemplars_new)[exemplars_new %in% as.numeric(representative_474_rnaseq[T.to.R.genes.80_Tak_rnaseq])],
    Kos_Tak = names(exemplars_new)[exemplars_new %in% as.numeric(representative_474_rnaseq[intersect(T.to.R.genes.80_Kos_rnaseq,T.to.R.genes.80_Tak_rnaseq)])]
)
...
for(i in 1:3){
  ...
  fn <- sprintf("../output/GO/TableS11_T.to.R.genes.80_%s_rnaseq_GO.csv",names(glist)[i]) 
...
### Table S11 T to R genes KEGG passway analysis ----
use.genelist <- rownames(dat)
...
for (i in 1:3){
  ...
  fn <- sprintf("../output/KEGG/TableS11_T.to.R.genes.80_%s_rnaseq_KEGG.csv", names(glist)[i]) 
...
gene_308 <- colnames(prob.env.choice.GC_Kos)[(prob.env.choice.GC_Kos[2,]-prob.env.choice.GC_Kos[1,] > 0)]
gene_378 <-  colnames(prob.env.choice.mix_Kos)[(prob.env.choice.mix_Kos[2,]-prob.env.choice.mix_Kos[1,] > 0)]

des_gene_308 <- des[gene_308, c(2,3,8)]
des_gene_378 <- des[gene_378, c(2,3,8)]

write.table(des_gene_308,file="../output/GO/TableS8_308_genes.txt",sep="\t",row.names = F) 
write.table(des_gene_378,file="../output/GO/TableS8_378_genes.txt",sep="\t", row.names = F) 
...
for (i in 1:6){
  ...
  fn <- sprintf("../output/GO/TableS9_GO_%s.csv",names(glist[i]))
  gl <-  glist[[i]] 
...
for (i in 1:6){
  ...
  fn <- sprintf("../output/KEGG/TableS9_KEGG_%s.csv",names(glist[i]))
  gl <-  glist[[i]] 
...
### Table S12 and 13 high performance in GC than field or in mix than field GO analysis Koshihikari ----
...
write.table(des_GC_mix_than_fld_0,file="../output/GO/TableS12_mix_GC_than_fld_genes.txt",sep="\t",row.names = F) 
write.table(des_GC_than_fld_0,file="../output/GO/TableS12_GC_than_fld_genes.txt",sep="\t",row.names = F) 
write.table(des_mix_than_fld_0,file="../output/GO/TableS12_mix_than_GC_genes.txt",sep="\t", row.names = F) 
...
for(i in 1:2){
  ...
  fn <- sprintf("../output/GO/TableS13_high_performance_in_%s.csv",names(glist)[i])
...
for (i in 1:2){
  ...
  fn <- sprintf("../output/KEGG/TableS13_high_performance_in_%s.csv",names(glist[i]))
  gl <-  glist[[i]] 
...
### Table S14 and 15 high performance in GC than field or in mix than field GO analysis Takanari ----
...
write.table(des_GC_mix_than_fld_0,file="../output/GO/TableS14_Takanari_mix_GC_than_fld_genes.txt",sep="\t",row.names = F) 
write.table(des_GC_than_fld_0,file="../output/GO/TableS14_Takanari_GC_than_fld_genes.txt",sep="\t",row.names = F) 
write.table(des_mix_than_fld_0,file="../output/GO/TableS14_Takanari_mix_than_GC_genes.txt",sep="\t", row.names = F) 
...
for(i in 1:2){
  ...
  fn <- sprintf("../output/GO/TableS15_Takanari_high_performance_in_%s.csv",names(glist)[i])
...
for (i in 1:2){
  ...
  fn <- sprintf("../output/KEGG/TableS15_Takanari_high_performance_in_%s.csv",names(glist[i]))
  gl <-  glist[[i]] 
