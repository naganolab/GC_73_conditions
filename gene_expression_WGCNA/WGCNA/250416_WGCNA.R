...
### Fig.3b differences in expression levels between Koshihikari and Takanari ----
...
data <- data.frame(xx=dif_MES[2:23])
...
### Fig.3c correlation between expression of eigengenes and trait ----
d_traits <- data.frame(at.lab.additional$average,at.lab.additional$Tmp)
cor_trait_eigengenes <- cor(d_traits, MES[2:23]) # 22 modules

cor.signif <- rep(NA,22)
for(i in 1:22){
  cor.signif[i] <- cor.test(d_traits[,2],MES[,i+1])$p.value
}
cor.signif.adjusted <- p.adjust(cor.signif, method = "BH")
...
ggsave(gggg, file="../output/Fig.3bc.pdf",
...
### fig
#j=20,k=35
pdf("../output/Fig.3d_Os01g0840100_35_20.pdf",width=2.3, height=1.3)
...
### fig
#j=35,k=20
pdf("../output/Fig.3d_Os01g0840100_20_35.pdf",width=2.3, height=1.3)
layout.mat2 <- matrix(c(1,2,3),byrow=F,nrow=1,ncol=3)
par(oma=c(3,3,3,3))
layout(layout.mat2,
       widths = c(2,2,2), heights = 1)
...
ggsave(g, file="../output/Fig.3e.pdf", # Fig.2e
...
fig_eigengenes <- function(i){
  ...
  ##########################
  ### L = na, D = 35-15C ###
  par(mar=c(0.5,0,0,0), mgp=c(1.5, 0.2, 0))
  for (j in c(35,30,25,20,15)) {
    ...
    col.v <- color5[1:2]
    plot(fig.df$Time - 24*floor(0.05*fig.df$Time) - 0.5, fig.df[,1], col=col.v[as.factor(fig.df$Cultivar)],
         ylim=y.lim.v, xaxt="n", yaxt = "n", cex=0.3)
    # # Y axis
    ...
    axis(2,at = c(-0.1, 0, 0.1),labels = FALSE, tck = -0.1)
    ...
    lines(xx,kos.est,col=color5[1], lwd=0.5)
    lines(xx,tak.est,col=color5[2], lwd=0.5)
  }
    ...
    for (k in c(20,25,30,35,40)) { # light temp
      ...
      if(j==k) {
        ...
        for (l in c(8,12,16)) {
          ...
          plot(fig.df$Time - 24*floor(0.05*fig.df$Time) - 0.5, fig.df[,1], col=col.v[as.factor(fig.df$Cultivar)],
               ylim=y.lim.v, cex=0.3, xaxt="n", yaxt="n")
          # # X axis
          # if((j==15&&l==8)||(j==15&&l==12)){
          #   axis(1, at = c(0, 15), cex.axis = 0.5)
          axis(1, at = c(0, 15), labels = FALSE, tck = -0.1)
          if(l==8){axis(2,at = c(-0.1, 0, 0.1),labels = FALSE, tck = -0.1)}
  ...
  # upper
  mtext(sprintf("module %s",i), side=3, outer=TRUE, line=2.8, cex=0.6)
...
dir.create("eigengenes")
for(i in 1:22){
pdf(sprintf("eigengenes/eigengene_panels_module_%s.pdf",i), width = 3.3, height = 3)
fig_eigengenes(i) 
dev.off()
}
...
ggsave(a, file="../output/Fig.3f.pdf", 
...
### Fig.3g export to cytoscape module25 ----
